...
async function adminAgentProfile(id) {
  ...
  showSheet('Данные агента',`<div class="stack"><p>${esc(x.full_name)}</p><p>${esc(x.phone)}</p><p>${esc(x.independent?'Частный агент':x.agency)}</p><p class="note">Данные сообщил агент. Заполнение профиля не подтверждает личность.</p></div>`,'','admin-agent',id);
}
