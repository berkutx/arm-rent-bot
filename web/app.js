...
async function submitSource(form,x){
  ...
  try{
    const fields={...post.fields,city:x.city.trim(),address:x.address.trim(),district:x.city.trim()==='Ереван'?x.district:'',kind:x.kind,rooms:x.rooms===''?null:Number(x.rooms),prices:[{amount:Number(x.amount),currency:x.currency,period:x.period}],role:post.market==='paid'?'agent':x.role,commission:post.market==='paid'?Number(x.commission):0,commission_type:fixed?'fixed':'percent',commission_currency:fixed?x.fee_unit:'AMD',commission_basis:x.fee_unit==='percent_day'?'day':'month'};
...
function connectCatalogEvents(){
  if(!state.sourceEnabled||document.hidden){catalogEvents?.close();catalogEvents=null;return;}
...
async function refreshVisible() {
  ...
  try {
    ...
    if(before?.source_revision){
      ...
      if(!after||after.source_revision!==before.source_revision){
        const show=()=>{if(after)void detail(openId);else {closeSheet();toast('Объявление снято с публикации.');}};
...
function drawMapPoint(index){
  ...
  $('#map-status').textContent=point.precision==='district'?'Примерно: район '+point.district+'.':point.precision==='street'?'Примерное расположение улицы.':'Примерное расположение по адресу.';
  rentalMap=L.map('rental-map',{scrollWheelZoom:false}).setView([point.lat,point.lon],point.precision==='district'?13:point.precision==='building'?17:15);
