...
with tempfile.TemporaryDirectory(prefix='rent-form-',ignore_cleanup_errors=True) as tmp:
 ...
 with sync_playwright() as p:
  ...
  expect(page.locator('#header [data-action=mine]')).to_be_visible()
  expect(page.locator('#header [data-action=nav][data-id=add]')).to_have_attribute('aria-label','Подать объявление')
  assert not page.locator('#header [data-action=open-admin],#header [data-id=alerts],#main [data-action=follow],#main [data-id=alerts]').count()
  page.locator('[data-action=conditions-filter]').click();assert page.locator('#filter-conditions input').count()==5
  ...
  page.locator('[data-action=conditions-filter]').click()
  page.locator('.sheet [data-action=follow]').click()
  page.wait_for_function('state.subs.length===1 && state.subs[0].active')
  saved=client.get('/api/subscriptions',headers=signed(username='')).json()
  assert len(saved)==1 and saved[0]['filters']['market']=='paid' and saved[0]['active']
  assert page.locator('#filter-conditions input').count()==4
  expect(page.locator('.sheet [data-action=follow]')).to_have_attribute('aria-checked','true')
  page.locator('#filter-conditions input[name=pets]').check()
  expect(page.locator('.sheet [data-action=follow]')).to_have_attribute('aria-checked','false')
  page.locator('.sheet [data-action=follow]').click()
  page.wait_for_function('state.subs.length===2 && state.subs.every(s=>s.active)')
  saved=client.get('/api/subscriptions',headers=signed(username='')).json()
  assert len(saved)==2 and all(x['active'] and x['filters']['market']=='paid' for x in saved)
  assert {x['filters']['pets'] for x in saved}=={True,False}
  pets_id=next(x['id'] for x in saved if x['filters']['pets'])
  for width in [320,390]:
   page.set_viewport_size({'width':width,'height':844})
   assert page.locator('.sheet-body').evaluate('(x)=>x.scrollWidth<=x.clientWidth')
   page.screenshot(path=str(OUT/f'filters-{width}.png'),animations='disabled')
  if page.locator('.sheet-head [data-action=close]').is_visible():
   page.locator('.sheet-head [data-action=close]').click();page.wait_for_function('!sheetKind')
  page.locator('[data-action=conditions-filter]').click()
  page.locator('.sheet [data-action=nav][data-id=alerts]').click()
  expect(page.locator('.subscription')).to_have_count(2)
  page.locator('[data-action=toggle-sub][data-id="'+pets_id+'"]').click()
  expect(page.locator('[data-action=toggle-sub][data-id="'+pets_id+'"]')).to_have_attribute('aria-checked','false')
  saved=client.get('/api/subscriptions',headers=signed(username='')).json()
  assert all(bool(x['active'])==(not x['filters']['pets']) for x in saved)
  page.locator('[data-action=delete-sub][data-id="'+pets_id+'"]').click()
  expect(page.locator('.subscription')).to_have_count(1)
  page.locator('[data-action=delete-sub]').click()
  expect(page.locator('.subscription')).to_have_count(0)
  assert client.get('/api/subscriptions',headers=signed(username='')).json()==[]
  page.locator('#header [data-action=back]').click();page.wait_for_function('state.screen==="feed"')
  page.locator('[data-action=conditions-filter]').click()
  page.locator('#filter-conditions input[name=pets]').uncheck()
  page.locator('button[form=filter-conditions]').click();page.wait_for_function('!sheetKind')
